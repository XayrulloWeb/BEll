...
export interface Bell {
  ...
  day: string;
  enabled: boolean;
  soundId: string | null; // <-- Убедимся, что он может быть null, как на бэкенде
  scheduleId: string;
  bellType: "lesson" | "break";
  breakDuration: number;
  autoGenerated?: boolean; // <-- Добавляем опциональное поле
  groupId?: string | null;
}
...
export interface StoreState {
  ...
  renameScheduleSet: (scheduleId: string, newName: string) => Promise<void>;
...
const useStore = create<StoreState>((set, get) => ({
  ...
  updateSchoolName: async (newName) => {
    try {
      ...
      const data = await response.json();
      if (!response.ok)
        throw new Error(data.message || "Не удалось обновить название");

      toast.success("Название школы успешно обновлено.");

      // Обновляем название школы в нашем состоянии superadmin
      set((state) => ({
        adminSchools: state.adminSchools.map((school) =>
  ...
  deleteScheduleSet: async (scheduleId: string) => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/schedules/schedules/${scheduleId}`,
        {
          method: "DELETE",
          headers: getAuthHeaders(),
        }
  ...
  renameScheduleSet: async (scheduleId, newName) => {
    ...
    set((state) => ({
      schedules: {
        ...state.schedules,
        [scheduleId]: { ...state.schedules[scheduleId], name: newName },
      },
    }));
    try {
      const response = await fetch(
      ...
      if (!response.ok) throw new Error("Не удалось переименовать расписание");
      toast.success("Расписание успешно переименовано.");
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Ошибка");
  ...
  updateBell: async (bellId, updatedData) => {
    ...
    set({
      schedules: { ...originalSchedules, [targetScheduleId]: updatedSchedule },
    });
  ...
  generateDayBells: async (params) => {
    ...
    if (action === "overwrite") {
      ...
      if (schedule) {
        const bellIdsToDelete = schedule.bells
        ...
        if (bellIdsToDelete.length > 0) {
          ...
          get().addLogEntry(
            `Очищено ${bellIdsToDelete.length} звонков для ${day}.`
          );
        }
      }
    }
    ...
    get().addLogEntry(
      `Сгенерировано расписание для ${lessonConfigs.length} уроков в дне: ${day}.`
    );
  ...
  fetchSpecialDays: async () => {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error("Не удалось загрузить данные календаря");
      const data = await response.json();
      set({ specialDays: data, isCalendarLoading: false });
    } catch (error) {
      toast.error(
        error instanceof Error ? error.message : "Ошибка загрузки календаря"
      );
  ...
  setSpecialDay: async (dayData) => {
    ...
    const existingRuleIndex = originalDays.findIndex(
      (d) => d.date === dayData.date
    );
    let newDays: SpecialDay[];
    if (existingRuleIndex > -1) {
      newDays = originalDays.map((d) =>
        d.date === dayData.date ? { ...d, ...optimisticDay } : d
      );
    } else {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(responseData.message || "Не удалось сохранить правило");
  ...
  deleteSpecialDay: async (date: string) => {
    ...
    try {
      ...
      const responseData = await response.json();
      if (!response.ok)
        throw new Error(responseData.message || "Не удалось удалить правило");
  ...
  createBellsBatch: async (bells) => {
    // Генерируем уникальные ID для каждого нового звонка прямо на фронтенде
    const bellsWithIds = bells.map((bell) => ({ ...bell, id: uuidv4() }));

    // Оптимистичное обновление
    set((state) => {
      const newSchedules = { ...state.schedules };
      // Добавляем новые звонки в соответствующие расписания
      bellsWithIds.forEach((bell) => {
        if (newSchedules[bell.scheduleId]) {
          newSchedules[bell.scheduleId].bells.push(bell as Bell);
        }
      });
    ...
    try {
      const response = await fetch(`${API_URL}/bells/batch`, {
        method: "POST",
        headers: getAuthHeaders(),
        body: JSON.stringify(bellsWithIds),
      });
      if (!response.ok) throw new Error("Не удалось скопировать звонки");
      toast.success("Звонки успешно скопированы.");
    } catch (error) {
      toast.error(
        error instanceof Error ? error.message : "Ошибка копирования"
      );
      // Откат состояния в случае ошибки (более сложная логика, пока можно опустить для простоты)
      // Для полноценного отката нужно было бы перезапросить все данные
  ...
  adminFetchUsers: async (schoolId: string) => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/admin/users?schoolId=${schoolId}`,
        { headers: getAuthHeaders() }
      );
