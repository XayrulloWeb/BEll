...
async function addBellToSchedule(
  bell: Omit<Bell, "autoGenerated"> & { autoGenerated?: boolean }
): Promise<Bell> {
  const newBell = await prisma.bell.create({
    data: { ...bell, autoGenerated: bell.autoGenerated || false },
  });
...
async function updateBell(
  bellId: string,
  bellData: Partial<Omit<Bell, "id" | "scheduleId" | "schoolId">>
): Promise<Bell | null> {
  try {
    const updatedBell = await prisma.bell.update({
      where: { id: bellId },
      data: bellData,
    });
    return updatedBell
      ? { ...updatedBell, soundId: updatedBell.soundId || "" }
      : null;
  } catch (error) {
    return null;
  }
...
async function deleteBell(bellId: string): Promise<boolean> {
  try {
    await prisma.bell.delete({ where: { id: bellId } });
    return true;
  } catch (error) {
    return false;
  }
...
async function createUser(
  userData: Omit<User, "id" | "passwordHash"> & { passwordHash: string }
): Promise<User> {
  const newUser = await prisma.user.create({ data: userData });
  return newUser as User;
}

async function getSpecialDaysForSchool(
  schoolId: string
): Promise<SpecialDay[]> {
  ...
  return days.map((d) => ({
    ...
    override_schedule_id: d.overrideScheduleId,
    type: d.type as "HOLIDAY" | "OVERRIDE",
  }));
}
...
async function setSpecialDay(data: SpecialDay): Promise<void> {
  await prisma.specialDay.upsert({
    ...
    create: {
      ...
      type: data.type,
      overrideScheduleId: data.override_schedule_id,
    },
  });
}
...
async function deleteSpecialDay(
  ...
  try {
    await prisma.specialDay.delete({
      where: { date_schoolId: { date, schoolId } },
    });
    return true;
  } catch (error) {
    return false;
...
async function getScheduleIdForToday(
  ...
  const specialDayRule = await prisma.specialDay.findUnique({
    where: { date_schoolId: { date: dateYYYYMMDD, schoolId } },
  });
  if (specialDayRule) {
    return specialDayRule.type === "HOLIDAY"
      ? null
      : specialDayRule.overrideScheduleId;
  }
...
async function getRingingBellsForSchedule(
  scheduleId: string,
  time: string,
  day: string
): Promise<Bell[]> {
  const bells = await prisma.bell.findMany({
    where: { scheduleId, time, day, enabled: true },
  });
  return bells.map((b) => ({ ...b, soundId: b.soundId || "" }));
}

async function getUsersBySchool(
  schoolId: string
...
async function createBellsBatch(bells: Bell[]): Promise<number> {
  const result = await prisma.bell.createMany({
    data: bells,
    skipDuplicates: true, // Пропускать, если звонок с таким ID уже существует
  });
  return result.count; // Возвращает количество успешно созданных записей
}
async function updateUserPassword(userId: string, newPasswordHash: string): Promise<boolean> {
    try {
...
async function updateSchoolName(schoolId: string, newName: string): Promise<School | null> {
    try {
...
async function getDataForSchool(
  schoolId: string
): Promise<{
  name: string;
  schedules: Record<string, FullSchedule>;
  sounds: Sound[];
  activeScheduleId: string | null;
} | null> {
  const school = await prisma.school.findUnique({
  ...
  for (const s of school.schedules) {
    schedules[s.id] = {
      ...s,
      schoolId: s.schoolId,
      bells: s.bells.map((b) => ({ ...b, soundId: b.soundId || "" })),
    };
  }
  ...
  // Вот правильная строка, которую вы прислали
  return { 
    name: school.name, 
    schedules, 
    sounds, 
